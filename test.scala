...
val userOrdersRDD = file.map(_.split(",")).map(key => (key(6).substring(1,key(6).length-1), (key(23).substring(1,14) + "," + key(1).substring(1,key(1).length-1) + "," + key(11).substring(1,key(11).length-1), key(13).substring(1,key(13).length-1))))
